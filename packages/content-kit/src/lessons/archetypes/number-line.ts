...
export function makeNumberLineLesson(spec: NumberLineLessonSpec): InteractiveLesson {
  ...
  const blocks = buildStandardBlocks({
    ...
    illustrationCaption: `Garis bilangan dari ${idNum(min)} sampai ${idNum(max)}; titik ${idNum(s0)} dan hasil lompatan ${idNum(s0 + j0)} ditandai.`,
    illustrationAlt: `Garis bilangan bertanda ${idNum(min)} hingga ${idNum(max)}; dua titik ditandai di ${idNum(s0)} dan ${idNum(s0 + j0)}.`,
    illustrationPrimitive: {
      name: 'NumberLineStrip',
      props: {
        title: `Garis bilangan ${idNum(min)}–${idNum(max)}`,
        min,
        max,
        step,
        highlightValues: [s0, s0 + j0],
      },
    },
